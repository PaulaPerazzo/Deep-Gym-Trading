...
def main():
    ...
    stock_data = pd.read_csv("./data/dji_stocks_adj_close_2023.csv")
    stock_data.drop(columns=["Unnamed: 0"], inplace=True)
    stock_data = stock_data.set_index("Ticker")

    index_data = pd.read_csv("./data/dow_jones_index_close_2023.csv")
    ...
    model_path = "./models/actor_critic_dow_jones.pth"
    ...
    for step, stocks in enumerate(selected_stocks):
        ...
        file_path = "./src/gym_trading/logs/testing_dow_jones.txt"
    ...
    file_path = "./src/gym_trading/logs/metrics_dow_jones.txt"
